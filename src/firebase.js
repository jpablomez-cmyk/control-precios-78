...
// onProgress receives { step, total, message }
export async function saveBatch(batch, onProgress) {
  try {
    ...
    await setDoc(doc(db, BATCHES_COL, id), {
      ...
      itemCount: items.length, chunkCount: chunks.length
    });
...
export async function updateBatchItems(batch, changedItems) {
  try {
    ...
    try { await saveBatch(batch); } catch (e2) { console.error("Fallback failed:", e2); }
  }
}
...
export async function loadBatches(onProgress) {
  try {
    ...
    for (let i = 0; i < snapshot.docs.length; i++) {
      ...
      const { itemCount, chunkCount, ...batchMeta } = data;
      batches.push({ id: batchDoc.id, ...batchMeta, items });
    }
    return batches;
...
export async function deleteBatchFromDB(batchId) {
  try {
    ...
    if (chunksSnap.docs.length > 0) {
      ...
      await wb.commit();
    } else { await deleteDoc(doc(db, BATCHES_COL, batchId)); }
